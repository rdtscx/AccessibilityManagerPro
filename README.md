...
- 闪退修复：
  - 授权通道联动检测改异步执行——Root 探测会 fork su 进程（最坏阻塞数秒），原在主线程执行会导致应用卡死/ANR 闪退，现改为后台线程探测、主线程启动服务，首页开关状态异步回调同步；
  - 事件日志全部改为单线程异步合并写——原先主线程/看门狗协程/通知监听系统线程同时写 SQLite 会触发 "database is locked" 崩溃且主线程被数据库 IO 阻塞（ANR），现内存缓冲 + 批量落库，调用线程零 IO、数据库零竞争。
- 运行逻辑与内存优化：
  - 通知等级/隐藏渠道读取加进程内缓存：通知监听在每条通知的高频路径上多次全量解析大 JSON，改为原始串不变即复用解析结果，显著降低 GC 与内存压力；
  - 事件日志批量写只做一次条数清理（原每条事件都 COUNT+DELETE），降低数据库 IO；
  - 自监控自动开启、看门狗等后台线程全部为守护线程，不阻碍进程退出。
- 新增本地崩溃日志（crash_log.txt）：未捕获异常自动落盘（含线程、堆栈、设备、版本），保留最近 5 条，纯本地不联网；下次闪退后把该文件发出来即可精确定位。
- 版本：versionCode 64 / versionName 6.0.0。

### v5.2.2

- 授权通道联动：检测到任一授权通道（ADB 授予 WRITE_SECURE_SETTINGS / Root / Shizuku）可用时，自动开启「无障碍自监控（低耗电）」开关并拉起服务，无需用户操作。检测时机覆盖：应用启动、开机/更新广播、无障碍服务重连、Shizuku 授权、进入首页。
- 尊重用户选择：在首页手动关闭过自监控后不再被自动开启；手动开启或一键优化后恢复通道联动能力。
- 自动开启为纯后台静默操作，不弹提示；无授权通道时不开启（避免无意义的常驻通知与引导）。

### v5.2.1

- 界面耦合：补齐「服务列表」「通知拦截历史」两个二级页的标题栏与返回键，二级页与首页、一级页在工具栏高度、配色、返回手势上统一，切换不再突兀；从首页进入子页面时标题栏会联动显示当前页名称，返回首页自动恢复。
- 自动化增强：无障碍服务被系统重新打开时自动补拉起自监控服务，避免保活闭环空窗。
- 修复：API 31+ 无 Root 时把后台保活应用误判为已死而误拉起的问题（补 UsageStats 存活判定）。
